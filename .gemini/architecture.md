...
│   ├── js/                 # Client-side Vanilla JS (main.js)
...
└── package.json            # Contains the core `build` script (`tsx render.tsx && vite build`)
```

## The Build Process (`npm run build`)
The entire site is generated via a custom script located in `render.tsx`. When you execute `npm run build`:
...
*   **Structured Data:** Every page config (`src/config/pages.ts`) defines an explicit `getStructuredData` function. When adding properties, ensure exact location data, capacity (`occupancy`), and features are meticulously documented in the JSON-LD payload.
*   **Crawler Guides:** Keep `public/llms.txt` manually updated if major architectural or property features change.