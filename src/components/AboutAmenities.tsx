...
export const AboutAmenities = ({ lang, propertyId, assetPrefix = './' }: { lang: Lang, propertyId: '1707' | '1606', assetPrefix?: string }) => {
  ...
  return (
    <section className="py-24 px-6 max-w-6xl mx-auto">
      <div className="grid md:grid-cols-2 gap-16">
        <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <img src={`${assetPrefix}${theme.images.bedroom}`} alt={t.aboutBedroomAlt} className="rounded-xl shadow-md h-48 w-full object-cover" loading="lazy" />
            <img src={`${assetPrefix}${theme.images.pool}`} alt={t.aboutPoolAlt} className="rounded-xl shadow-md h-48 w-full object-cover" loading="lazy" />
          </div>
        </div>
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {[
              ...
              <div key={index} className={`flex items-center space-x-4 p-4 bg-white rounded-xl shadow-sm border ${theme.borderColor} hover:shadow-md transition-shadow`}>
                <div className={`${theme.aboutThemeAccent} p-2 rounded-lg`}>{item.icon}</div>
      ...
      <div className={`mt-20 p-8 md:p-12 ${theme.aboutHostBg} rounded-3xl text-white flex flex-col md:flex-row items-center gap-10`}>
        <div className={`w-32 h-32 md:w-48 md:h-48 rounded-full overflow-hidden flex-shrink-0 border-4 ${theme.aboutHostBorder}`}>
           <img 
        ...
        <div>
           <h2 className={`text-sm font-bold uppercase tracking-widest ${theme.aboutHostAccent} mb-2`}>{t.hostTitle}</h2>
           <h3 className={`text-3xl font-bold mb-4 ${theme.aboutHostName}`}>{t.hostName}</h3>
