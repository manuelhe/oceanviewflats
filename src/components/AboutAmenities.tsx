...
export const AboutAmenities = ({ lang, assetPrefix = './' }: { lang: Lang, assetPrefix?: string }) => {
  const t = dict[lang];
  ...
  return (
    <section className="py-24 px-6 max-w-6xl mx-auto">
      <div className="grid md:grid-cols-2 gap-16">
        ...
        <div>
          <h2 className="text-3xl font-bold mb-8 text-slate-900">{t.amenitiesTitle}</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {[
              ...
              <div key={index} className="flex items-center space-x-4 p-4 bg-white rounded-xl shadow-sm border border-slate-100 hover:shadow-md transition-shadow">
                <div className="text-[#FF5A5F] bg-red-50 p-2 rounded-lg">{item.icon}</div>
      ...
      <div className="mt-20 p-8 md:p-12 bg-slate-900 rounded-3xl text-white flex flex-col md:flex-row items-center gap-10">
        <div className="w-32 h-32 md:w-48 md:h-48 rounded-full overflow-hidden flex-shrink-0 border-4 border-slate-700">
           <img 
        ...
        <div>
           <h2 className="text-sm font-bold uppercase tracking-widest text-[#FF5A5F] mb-2">{t.hostTitle}</h2>
           <h3 className="text-3xl font-bold mb-4">{t.hostName}</h3>
